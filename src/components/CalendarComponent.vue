            ...
            <t-collapse :borderless="true" style="width: 100%">
              <t-collapse-panel value="0" header="今日待办">
                ...
                <t-space direction="vertical">
                  ...
                  >
                    <span :style="{ color: todo.color, textDecoration: todo.isComplete ? 'line-through' : 'none' }">
                      {{ todo.description }}
                    </span>
                    <t-button
                      shape="circle"
           ...
           <template #panel>
             <ul class="tdesign-demo__select-input-ul-single">
               <li v-for="item in options" :key="item.value" @click="() => onOptionClick(item)">
                 <div :style="{ backgroundColor: item.label, height: '15px', width: '15px', borderRadius:'50%' }"></div>
               </li>
             </ul>
           </template>
           <template #suffixIcon>
    ...
    <t-space direction="vertical" style="width: 100%">
      ...
      <div style="display: flex;">
        ...
        >
          <template #panel>
            <ul class="tdesign-demo__select-input-ul-single">
              <li v-for="item in options2" :key="item.value" @click="() => onOptionClick2(item)">
                <div :style="{ backgroundColor: item.label, height: '15px', width: '15px', borderRadius:'50%' }"></div>
              </li>
            </ul>
          </template>
