...
const columns = ref([
  {
    ...
    cell: (h, { row }) => {
      return (
        <t-icon name={currentQuestionState[row.status].icon} class={currentQuestionState[row.status].theme} />
      )
    },
    width: '70'
  },
  ...
  { colKey: 'passRate', title: '通过率', ellipsis: true },
  {
    ...
    cell: (h, { row }) => {
      return (
        <t-tag shape="round" theme={currentQuestionDifficult[row.Difficulty].theme} variant="light-outline">
